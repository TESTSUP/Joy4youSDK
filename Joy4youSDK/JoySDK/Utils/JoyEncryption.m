...
+ (NSDictionary *)DESDecryptData:(NSData *)data WithKey:(NSString *)key
{
    if ([data length] && [key length]) {
        ...
        NSError *error = nil;
        NSString *temp = [[NSString alloc] initWithBytes:[decData bytes]
                                                  length:[decData length]
                                                encoding:NSUTF8StringEncoding];
        JYDLog(@"request response string = %@", temp);
...
{
    if ([dataStr length] && [key length]) {
        NSData *orgData = [dataStr dataUsingEncoding:NSUTF8StringEncoding];
...
+ (NSData *)dataFromHexString:(NSString *)hexString { //
    ...
    NSData *data = [[NSData alloc] initWithBytes:myBuffer length:length];
    free(myBuffer);
