...
@interface JYAlertView () <JCViewControllerDelegate>
...
@property (nonatomic, weak) JYAlertViewController *vc;
...
@interface JYSingleTon : NSObject

@property (nonatomic, strong) UIWindow *backgroundWindow;
@property (nonatomic, weak) UIWindow *oldKeyWindow;
@property (nonatomic, strong) NSMutableArray *alertStack;
@property (nonatomic, strong) JYAlertView *previousAlert;

@end

@implementation JYSingleTon

+ (instancetype)shareSingleTon{
    static JYSingleTon *shareSingleTonInstance = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        shareSingleTonInstance = [JYSingleTon new];
    });
...
@property (nonatomic, weak) JYAlertView *alertView;
...
- (void)addCoverView{
    ...
    self.coverView.autoresizingMask =
    UIViewAutoresizingFlexibleLeftMargin |
    UIViewAutoresizingFlexibleRightMargin |
    UIViewAutoresizingFlexibleTopMargin |
    UIViewAutoresizingFlexibleBottomMargin |
...
- (void)show{
    [[JYSingleTon shareSingleTon].alertStack addObject:self];
...
+ (void)showOneButtonWithTitle:(NSString *)title Message:(NSString *)message ButtonType:(JYAlertViewButtonType)buttonType ButtonTitle:(NSString *)buttonTitle Click:(clickHandle)click{
    ...
    JYAlertView *alertView = [JYAlertView new];
...
- (void)showAlert{
    ...
    if (count > 1) {
        NSInteger index = [[JYSingleTon shareSingleTon].alertStack indexOfObject:self];
        previousAlert = [JYSingleTon shareSingleTon].alertStack[index - 1];
    }
...
- (void)showAlertHandle{
    ...
    if (keywindow != [JYSingleTon shareSingleTon].backgroundWindow) {
        [JYSingleTon shareSingleTon].oldKeyWindow = [UIApplication sharedApplication].keyWindow;
    }
    
    JYAlertViewController *vc = [[JYAlertViewController alloc] init];
    vc.delegate = self;
    vc.alertView = self;
    self.vc = vc;
...
- (void)dismissAlertWithCompletion:(void(^)(void))completion{
    [self.vc hideAlertWithCompletion:^{
        ...
        NSInteger count = [JYSingleTon shareSingleTon].alertStack.count;
        if (count > 0) {
            JYAlertView *lastAlert = [JYSingleTon shareSingleTon].alertStack.lastObject;
            [lastAlert showAlert];
        }
    }];
}
...
- (void)stackHandle{
    [[JYSingleTon shareSingleTon].alertStack removeObject:self];
...
- (void)toggleKeyWindow{
    [[JYSingleTon shareSingleTon].oldKeyWindow makeKeyAndVisible];
    [JYSingleTon shareSingleTon].backgroundWindow.rootViewController = nil;
    [JYSingleTon shareSingleTon].backgroundWindow.frame = CGRectZero;
}
